...
const updateTrades = async (app, season, week) => {
    ...
    for (let j = 0; j < increment; j += 25) {
        await Promise.all(leagues_to_update.filter(l => l.rosters.find(r => r?.players?.length > 0)).slice(j, j + 25).map(async league => {
            ...
            try {
                let transactions_league = await axios.get(`https://api.sleeper.app/v1/league/${league.league_id}/transactions/${week}`);
...
exports.season_trades = async (app) => {
    app.set('syncing', true);

    console.log('Beginning SEASON Trade Sync...')

    const state = app.get('state')


    for (let i = 1; i <= 7; i++) {
        await updateTrades(app, state.season, i)
    }
    app.set('syncing', false)
    console.log('SEASON Trade Sync Complete...')
}